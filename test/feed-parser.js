...
describe('feed parser', function () {
  ...
  it('should retrieve description from feed', function() {
    ...
    const promises = xmls.map((xml, i) => {
      return expect(parser(xml)).to.eventually.have.property('description').that.equal(expected[i]);
    });
  ...
  it('should retrieve title from feed', function() {
    ...
    const promises = xmls.map((xml, i) => {
      return expect(parser(xml)).to.eventually.have.property('title').that.equal(expected[i]);
    });
  ...
  it('should generate url', function() {
    ...
    const promises = xmls.map((xml, i) => {
      return expect(parser(xml)).to.eventually.have.property('url').that.equal(expected[i]);
    });
